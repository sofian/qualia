...
//  chargeBalance += agent.getCharge();
  ...
  //p.velocity().set( 0.1*(mouseX-p.position().x()), 0.1*(mouseY-p.position().y()), 0);
...
void initMapper() {
  ...
  dev.add_input("/accel_x", 1, 'f', "", new Double(0.0), null, 
    new InputListener() {
  ...
  dev.add_input("/accel_y", 1, 'f', "", new Double(0.0), null, 
    new InputListener() {
      public void onInput(float[] v) {
        vy = vy*0.99 + v[0];
      }
